...
export default function CustomersPage() { 
    ...
    async function onUpdate(e: React.FormEvent) {
        ...
        await updateMut.mutateAsync({
            id: editingId,
            dto: { fullName: editFullName, email: editEmail },
        });
    ...
    return ( 
        <div className="min-h-screen bg-slate-50"> 
            ...
            <main className="mx-auto max-w-5xl px-4 py-6 space-y-4"> 
                ...
                {editingId !== null && (
                    <form onSubmit={onUpdate} className="rounded-xl border bg-blue-50 p-4 space-y-3">
                        <p className="text-sm text-slate-600">
                            <b>Mutation (PUT)</b>: actualiza customer y luego invalida cache para refrescar listado.
                        </p>
                        ...
                        {updateMut.isError && (
                            <p className="text-sm text-red-600">Error actualizando: {String(updateMut.error)}</p>
                        )}
                    </form>
                )} 
                                ...
                                {data.map((c: Customer) => ( 
                                    <tr key={c.id} className="border-t"> 
                                        ...
                                        <td className="p-3 flex gap-2"> 
                                            <button 
                                                className="rounded-md border px-2 py-1 hover:bg-blue-50 disabled:opacity-50"
